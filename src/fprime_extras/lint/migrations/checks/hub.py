...
from fprime_extras.lint.migrations.model.patcher import Port
...
class HubLinter(CheckBase):
    """ Hub linter """
    PORT_PAIRS = {"portIn": "portOut", "portOut": "portIn",
                  "buffersIn": "buffersOut", "buffersOut": "buffersIn"}
    ...
    def get_identifiers(cls) -> Dict[str, CheckSeverity]:
        ...
        for delegate in cls.DELGATED_CHECKS:
            notifiers.update({"hub-{}".format(key): value for key,
                              value in delegate.get_identifiers().items()})
        notifiers.update({
    ...
    def get_sorted_hub_ports(cls, model: Topology, hub_name: str):
        ...
        for port_name in cls.PORT_PAIRS.keys():
            ...
            hub_ports = [get_port_by_name(hub_object, port_name, num) for num in range(
                0, int(port_zero.get_max_number()))]
            cmp_ports = [cls.get_component_port(
                model, port) for port in hub_ports]
    ...
    def run(self, result: CheckResult, model: Topology, extras: List[str]) -> CheckResult:
        ...
        # we will do terrible things to this topology
        local_model = copy.deepcopy(model)

        # Args validation
        if get_comp_by_name(local_model, local_hub_name) is None:
            raise Exception(
                "Could not find component '{}' in local topology".format(local_hub_name))
        elif get_comp_by_name(remote_model, remote_hub_name) is None:
            raise Exception(
                "Could not find component '{}' in remote topology".format(remote_hub_name))

        # Get comparable sets
        local_ports_dict = self.get_sorted_hub_ports(
            local_model, local_hub_name)
        remote_ports_dict = self.get_sorted_hub_ports(
            remote_model, remote_hub_name)
        ...
        for port_key_local, port_key_remote in self.PORT_PAIRS.items():
            ...
            sequence = [(pair[0][0], pair[0][1], pair[1][0], pair[1][1])
                        for pair in combined_tmp]
            ...
            for local_hub_port, local_comp_port, remote_hub_port, remote_comp_port in sequence:
                ...
                # If a local port is connected but the remote hub port is not connected
                    ...
                    result.add_problem("hub-ports-not-parallel",
                                       ...
                                       .format(local_comp_port.get_type(), CheckBase.get_standard_identifier(local_model, local_hub_name, local_hub_port),  # noqa: E501
                                               CheckBase.get_standard_identifier(remote_model, remote_hub_port.get_target_comp(
                                               ), remote_comp_port), remote_comp_port.get_type(),
                                               CheckBase.get_standard_identifier(remote_model, remote_hub_name, remote_hub_port)),
                                       local_model, local_hub_port.get_target_comp(), local_comp_port)
                    ...
                    result.add_problem("hub-port-directions-not-parallel",
                                       ...
                                       .format(local_hub_port.get_direction(),
                                               CheckBase.get_standard_identifier(
                                                   remote_model, remote_hub_name, remote_hub_port),
                    ...
                    result.add_problem("hub-port-directions-not-parallel",
                                       ...
                                       .format(local_comp_port.get_direction(), CheckBase.get_standard_identifier(local_model, local_hub_name, local_hub_port),  # noqa: E501
                                               CheckBase.get_standard_identifier(remote_model, remote_hub_port.get_target_comp(
                                               ), remote_comp_port), remote_comp_port.get_direction(),
                                               CheckBase.get_standard_identifier(remote_model, remote_hub_name, remote_hub_port)),
                                       local_model, local_hub_port.get_target_comp(), local_comp_port)
        ...
        for port_key_local, port_key_remote in self.PORT_PAIRS.items():
            ...
            for local_hub_port, local_comp_port, remote_hub_port, remote_comp_port in sequence:
                ...
                if local_comp_port is not None:
                    local_comp_port.set_target_direction(
                        remote_hub_port.get_target_direction())
                    local_comp_port.set_target_num(
                        remote_hub_port.get_target_num())
                    local_comp_port.set_target_type(
                        remote_hub_port.get_target_type())
                    local_comp_port.set_target_comp(
                        remote_hub_port.get_target_comp())
                    local_comp_port.set_target_port(
                        remote_hub_port.get_target_port())
                    # Clear hub ports
                ...
                if remote_comp_port is not None:
                    remote_comp_port.set_target_direction(
                        local_hub_port.get_target_direction())
                    remote_comp_port.set_target_num(
                        local_hub_port.get_target_num())
                    remote_comp_port.set_target_type(
                        local_hub_port.get_target_type())
                    remote_comp_port.set_target_comp(
                        local_hub_port.get_target_comp())
                    remote_comp_port.set_target_port(
                        local_hub_port.get_target_port())
                    # Clear hub ports
