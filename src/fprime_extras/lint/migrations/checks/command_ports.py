...
class CommandPorts(CheckBase):
    ...
    def run(self, result: CheckResult, model: Topology, extras: List[str]) -> CheckResult:
        ...
        for component in model.get_comp_list():
            component_mappings[component.get_name()] = {
                key: None for key in self.CMD_TYPES.keys()}
            ...
            for port in component.get_ports():
                ...
                target_port_obj = get_port_by_name(
                    target_comp_obj, port_target_port, port.get_target_num())
                component_mappings[component.get_name(
                )][port_type] = target_port_obj
