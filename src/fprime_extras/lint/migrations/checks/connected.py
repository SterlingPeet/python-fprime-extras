        ...
        for component in model.get_comp_list():
            for port in component.get_ports():
                ...
                if port.get_target_comp() is None or port.get_target_comp() == "":
                    result.add_problem(
                        "port-not-connected", "is not connected", model, component, port)
                    continue
                try:
                    target_comp = get_comp_by_name(
                        model, port.get_target_comp())
                    if target_comp is None:
                        result.add_problem("invalid-target-component-in-connection",
                                           "connected to non-existent component {}".format(
                                               port.get_target_comp()),
                                           model, component, port)
                        continue
                    target_port = get_port_by_name(
                        target_comp, port.get_target_port(), port.get_target_num())
                    if target_port is None:
                        result.add_problem("invalid-target-port-in-connection",
                                           ...
                                           model, component, port)
                        continue
                    max_num = 1 if target_port.get_max_number() is None else int(
                        target_port.get_max_number())
