...
from fprime_ac.utils.buildroot import get_build_roots
from fprime_ac.utils.buildroot import search_for_file
from fprime_ac.utils.buildroot import set_build_roots
from fprime_extras.lint.migrations.settings import IniSettings
...
def load_patched_topology(topology_xml: Path, settings_dir: Path = None) -> Topology:
    ...
    try:
        ...
        base_locations = [settings.get(
            "framework_path"), settings.get("project_root", None)]
        ...
        set_build_roots(":".join(
            [str(location) for location in base_locations if location is not None]))
        ac_consts = Path(settings.get("ac_constants", Path(
            settings.get("framework_path")) / "config" / "AcConstants.ini"))
        if ac_consts and ac_consts.exists():
            os.environ["FPRIME_AC_CONSTANTS_FILE"] = str(ac_consts)
        else:
            ac_consts = Path(settings.get("ac_constants", Path(
                settings.get("framework_path")) / "Fw" / "Cfg" / "AcConstants.ini"))
            if ac_consts and ac_consts.exists():
                os.environ["FPRIME_AC_CONSTANTS_FILE"] = str(ac_consts)
        _log.info('Loaded AC Constants File: {}'.format(ac_consts))
        ...
        try:
            ...
            raise inc  # Pass through if already inconsistency exception
...
def __topology_loader(topology_xml: Path) -> Topology:
    ...
    imported_comps = [__component_loader(comp_xml) for comp_xml in set(
        parsed_topology.get_component_includes())]
...
def __component_loader(comp_xml: Path) -> Component:
    ...
    port_pairs = [__port_loader(port_file) for port_file in set(
        parsed_comp_xml.get_port_type_files())]
    ser_xmls = [__ser_loader(
        ser_file) for ser_file in parsed_comp_xml.get_serializable_type_files()]
    port_objs = {pair[0].get_type(): pair[0] for pair in port_pairs}
    port_objs["Serial"] = Port(None, "Serial", None)

    comp = CompFactory.getInstance().create(
        parsed_comp_xml, [pair[1] for pair in port_pairs], ser_xmls)
    ...
    for attr in set(list(amalgam.__dict__.keys()) + list(traits.__dict__.keys())):
        ...
        # Recurse for ports
            ...
            for port in amalgam._Component__port_obj_list:
                trait_port = get_port_by_name(
                    traits, port.get_name(), port.get_source_num())
                if trait_port is None:
                    continue
                __merge_object_model(port, trait_port)
            new_ports = [port for port in traits._Component__port_obj_list if get_port_by_name(
                amalgam, port.get_name(), port.get_source_num()) is None]
        ...
        # Code uses different names
            ...
            pass
        elif amalgam_value is None or amalgam_value == "" or (isinstance(amalgam_value, (list, tuple)) and len(amalgam_value) == 0) or not amalgam_value:  # noqa: E501
            pass
            ...
            raise InconsistencyException(
                "{} has inconsistent definitions ".format(amalgam.get_name()))
...
def get_comp_by_name(model: Topology, name: str) -> Component:
    ...
    if not comps:
        ...
        raise InconsistencyException(
            "Multiple components with name {} defined".format(name))
    return comps[0]
