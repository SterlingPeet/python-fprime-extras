...
from fprime_extras.lint.migrations.model.patcher import Component
...
class CheckResult:
    ...
    def add_problem(self, ident: str, message: str, top: Union[Topology, str] = None, comp: Union[Component, str] = None, port: Union[Port, str] = None):  # noqa: E501
        ...
        """
        assert ident in self.severities, "{} is not in list of expected identifiers".format(
            ident)
    ...
    def get_filtered_problems(self, filters: List[Dict[str, Union[str, List[Pattern]]]]) -> List[dict]:
        ...
        def is_excluded(problem):
            ...
            return False
        problems = [
            problem for problem in self.problems if not is_excluded(problem)]
        return problems
...
class CheckBase(abc.ABC):
    ...
    @staticmethod
    def get_standard_identifier(topology: Union[Topology, str] = None, component: Union[Component, str] = None, port: Union[Port, str] = None) -> str:  # noqa: E501
        ...
        """
        tokens = [item if isinstance(item, str) else item.get_name() for item in [
            topology, component, port] if item is not None]
    ...
    def get_identifiers(self) -> Dict[str, CheckSeverity]:
        ...
        """
        raise NotImplementedError("Must implement this method of the checker")
    ...
    def get_checkers(cls, excluded: List[str] = None, candidate: Type = None) -> List[Type["CheckBase"]]:
        ...
        checkers = [candidate] if not inspect.isabstract(
            candidate) and candidate.__name__ not in excluded else []
    ...
    def run_all(cls, model: Topology, excluded: List[str] = None, filters: List[str] = None, arguments=None):
        ...
        checkers = cls.get_checkers(excluded)
        _log.warning("[FP-LINT] Found {} checks".format(len(checkers)))
        all_clear = True
        for checker in checkers:
            try:
                ...
                if needed_args and arguments is None or len(needed_args) != len(filled_args):
                    ...
                    continue
                _log.warning("[FP-LINT] Running check '{}'".format(checker.__name__))
                ...
                for problem in problems:
                    _log.warning(result.get_problem_message(problem))
                all_clear = all_clear and not problems
            except Exception as exc:
                _log.error("[ERROR] {} failed: {}".format(
