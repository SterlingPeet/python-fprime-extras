...
def load_configuration(yml_file, known_idents: List[str]) -> Dict[str, List[Dict[str, Union[str, List[Pattern]]]]]:
    ...
    identifiers = [__homogenize_filter(
        known_idents, ident, prop) for ident, prop in config.get("filters", {}).items()]
    config["filters"] = identifiers
    return config


def __homogenize_filter(idents: List[str], identifier: str, properties: List[dict]) -> Dict[str, Union[str, List[Pattern]]]:
    ...
    if identifier not in idents:
        raise ConfigurationException(
            "Unknown identifier '{}' specified".format(identifier))
    properties = [{}] if not properties else properties
    specifiers = [re.compile(prop.get("specifier", ".*"))
